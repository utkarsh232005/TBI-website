...
export default function HeroSection({ onApplyClick }: HeroSectionProps) {
  ...
  const { resolvedTheme } = useTheme(); // Keep for other theme-dependent logic if any
  ...
  const handleCampusStatusSelect = (status: "campus" | "off-campus") => {
    ...
    // For campus applicants, proceed with the on-site form
    if (onApplyClick) {
      onApplyClick();
    } else {
      // Fallback or error handling if onApplyClick is not provided
  ...
  return (
    <section 
      className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background text-foreground pt-12 sm:pt-16" // Changed bg-black to bg-background
    >
      ...
      >
        <p 
          ref={taglineRef}
          className="text-sm font-normal tracking-widest uppercase mb-8 opacity-0 text-foreground" // Ensured base text color is theme-aware
        >
        ...
        <h1 
          ref={headlineRef}
          className="font-orbitron text-4xl font-normal text-foreground sm:text-5xl lg:text-6xl xl:text-7xl" // Changed text-white to text-foreground
        >
        ...
        >
          <div className="relative inline-flex items-center justify-center w-full sm:w-auto group">
            ...
            <Button
              ...
              className="relative inline-flex items-center justify-center w-full px-8 py-3 text-base font-normal text-foreground bg-background border border-transparent rounded-full sm:w-auto group" // Changed bg-black text-white to bg-background text-foreground
            >
          ...
          <Button
            asChild
            variant="outline" // Outline variant will use theme border/text colors
            size="lg"
            className="inline-flex items-center justify-center w-full px-8 py-3 text-base font-normal text-foreground transition-all duration-200 bg-background border-border rounded-full sm:w-auto hover:border-primary group" // Changed specific colors to theme-aware
          >
            <a href="#startups">
              See Success Stories
            </a>
